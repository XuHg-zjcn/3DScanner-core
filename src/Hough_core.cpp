...
//双线性插值 output 24bit
uint32_t Hough_core::BiLinear(uint64_t *pNear4, uint32_t x, uint32_t y) {
    uint32_t sum=0;
...
//获取用于双线性插值的四个点
void Hough_core::get_near4(xy<uint32_t> *pH16, uint64_t *pNear4) {
...
void Hough_core::lines_search() {
    ...
    point_u32 point = point_u32(0, 0);      //无用
    xy<int> delta_point{0,0};               //线上点的间距
    xy<int> delta_line{0,0};                //平行线的间距
    rad += paras.rads.step * paras.rads.start;
